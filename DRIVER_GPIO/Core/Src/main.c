...
/* Includes ------------------------------------------------------------------*/
#include "main.h"
#include "API_GPIO.h"  // Aqui incluyo el driver GPIO
#include "string.h"
...
int main(void)
{
  ...
  /* USER CODE END 2 */
  /*codigo agregado al original (app1.1)*/
  // Vector de LEDs
  ...
  while (1)
  {
	  	  // Leer el estado del pulsador usando el driver GPIO
	      ...
	      // Control de LEDs usando las funciones del driver
	              if (current_led == 0) {
	                  LED_On(LD1);  // Enciende LED 1 (verde)
	                  LED_Off(LD2);  // Apaga LED 2 (azul)
	                  LED_Off(LD3);  // Apaga LED 3 (rojo)
	              } else if (current_led == 1) {
	                  LED_Off(LD1);  // Apaga LED 1 (verde)
	                  LED_On(LD2);   // Enciende LED 2 (azul)
	                  LED_Off(LD3);  // Apaga LED 3 (rojo)
	              } else if (current_led == 2) {
	                  LED_Off(LD1);  // Apaga LED 1 (verde)
	                  LED_Off(LD2);  // Apaga LED 2 (azul)
	                  LED_On(LD3);   // Enciende LED 3 (rojo)
	              }
