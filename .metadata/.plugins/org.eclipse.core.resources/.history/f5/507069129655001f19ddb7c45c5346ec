...
void Secuencia3() {
	    ...
	    const int led1_interval = 100; // LED 1 parpadea cada 100ms
	    const int led2_interval = 300; // LED 2 parpadea cada 300ms
	    const int led3_interval = 600; // LED 3 parpadea cada 600ms

	    // Variables para almacenar el último tiempo de cambio de estado
	    ...
	    GPIO_PinState led1_state = GPIO_PIN_RESET;
	    GPIO_PinState led2_state = GPIO_PIN_RESET;
	    ...
	    while (1)
	    {
	        ...
	        // Salir de la secuencia 3 si se presiona el botón
	         if (button_state == GPIO_PIN_SET && last_button_state == GPIO_PIN_RESET)
	            {
	             last_button_state = button_state;
	             break; // Terminar el bucle para finalizar la secuencia 3
	             }
	             last_button_state = button_state;

	        // Controlar el parpadeo del LED 1
	        if (current_time - led1_last_toggle >= led1_interval)
	        {
	            led1_state = (led1_state == GPIO_PIN_SET) ? GPIO_PIN_RESET : GPIO_PIN_SET;
	            HAL_GPIO_WritePin(LED_PORTS[0], LED_PINS[0], led1_state);
	            led1_last_toggle = current_time;
	        }

	        // Controlar el parpadeo del LED 2
	        if (current_time - led2_last_toggle >= led2_interval)
	        {
	            led2_state = (led2_state == GPIO_PIN_SET) ? GPIO_PIN_RESET : GPIO_PIN_SET;
	            HAL_GPIO_WritePin(LED_PORTS[1], LED_PINS[1], led2_state);
	            led2_last_toggle = current_time;
	        }

	        // Controlar el parpadeo del LED 3
	        if (current_time - led3_last_toggle >= led3_interval)
	        {
	            led3_state = (led3_state == GPIO_PIN_SET) ? GPIO_PIN_RESET : GPIO_PIN_SET;
	            HAL_GPIO_WritePin(LED_PORTS[2], LED_PINS[2], led3_state);
